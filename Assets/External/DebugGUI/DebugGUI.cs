...
public class DebugGUI : MonoBehaviour
{
    ...
    void DrawGraphs()
    {
        ...
        {
            GUI.Box(new Rect(
                transform.position.x - 3 - graphLabelWidth - 5,
                transform.position.y + group * graphBlockHeight,
                graphLabelWidth + 5,
                graphHeight
            ), "", boxStyle);
        }
        ...
        foreach (GraphContainer graph in graphDictionary.Values)
        {
            ...
            {
                GUI.Box(new Rect(transform.position.x, transform.position.y + graphBlockHeight * graph.group, graphWidth, graphHeight), "", boxStyle);
            }

            graph.Draw(new Rect(transform.position.x, transform.position.y + graphBlockHeight * graph.group, graphWidth, graphHeight));
        }

        foreach (int group in graphGroupBoxesDrawn)
        {
            float groupOrigin = transform.position.y + group * graphBlockHeight;
            ...
            foreach (GraphContainer graph in graphDictionary.Values)
            {
                ...
                if (graph.group == group)
                {
                    ...
                    GUI.Label(new Rect(transform.position.x - minMaxXOffset, transform.position.y + group * graphBlockHeight, minMaxXOffset, graphHeight), labelGuiContent, minMaxTextStyle);
                    // Min
                    labelGuiContent.text = graph.min.ToString("F2");
                    GUI.Label(new Rect(transform.position.x - minMaxXOffset, transform.position.y + group * graphBlockHeight + graphHeight - textSize.y, minMaxXOffset, graphHeight), labelGuiContent, minMaxTextStyle);
                    ...
                    GUI.Label(new Rect(transform.position.x - xOffset, yOffset, xOffset, graphHeight), labelGuiContent);
        ...
        foreach (var group in graphGroupBoxesDrawn)
        {
            ...
            if (
                transform.position.x < mousePos.x && mousePos.x < transform.position.x + graphWidth &&
                transform.position.y + group * graphBlockHeight < mousePos.y && mousePos.y < transform.position.y + group * graphBlockHeight + graphHeight
            )
            {
                ...
                GUI.Box(new Rect(mousePos.x, transform.position.y + group * graphBlockHeight, 1, graphHeight), "", boxStyle);

                // Background box
                GUI.backgroundColor = new Color(0, 0, 0, 0.5f);
                GUI.color = Color.white;
                GUI.Box(new Rect(mousePos.x - 60, transform.position.y + group * graphBlockHeight, 55, 55), "", boxStyle);
