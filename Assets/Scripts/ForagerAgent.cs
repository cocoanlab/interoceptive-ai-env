...
public class ForagerAgent : Agent
{
    ...
    private int num_resource_red = 400;
    private int num_resource_blue = 400;
    //private int num_resource_red = 200;
    //private int num_resource_blue = 200;
    //public int num_resource_red = (int)Academy.Instance.EnvironmentParameters.GetWithDefault("num_resources", 400.0f);
    //public int num_resource_blue = (int)Academy.Instance.EnvironmentParameters.GetWithDefault("num_resources", 400.0f);
    private int num_tree = 100;
    private float[] resource_levels;
    private GameObject[] objects_red;
    private GameObject[] objects_blue;
    private GameObject[] objects_tree;
    private float objects_initial_height= 3.0f;
    private float objects_initial_range = 60.0f;
    ...
    void Start()
    {
        ...
        this.objects_tree = new GameObject[this.num_tree];
        for (int i = 0; i < this.num_tree; i++)
        {
            GameObject obj = Instantiate(prefab_tree) as GameObject;
            objects_tree[i] = obj;
            objects_tree[i].transform.position = GetPos();
        }
    ...
    public override void OnEpisodeBegin()
    {
        ...
        for (int i = 0; i < this.num_tree; i++)
        {
            Vector3 pos = GetPos(-3.0f);
            objects_tree[i].transform.position = pos;
        }
    }
