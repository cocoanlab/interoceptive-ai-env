...
public class DynamicWeatherSystem : MonoBehaviour
{
    ...
    IEnumerator StartDynamicWeather()
    {
        while (true)
        {
            ...
            {
                SelectWeather();
                ChangeFieldTemperature(); // Call ChangeFieldTemperature when weatherState changes
            }
            ...
            yield return null;
            //     Debug.Log("weather state:  " + weatherState);
        }
    }
    ...
    void ChangeFieldTemperature()
    {
        fieldThermoGrid.SetParameters();

        float defaultTemperature = fieldThermoGrid.GetFieldDefaultTemp();

        float adjustedTemperature = 0.0f;


        if (weatherState == WeatherState.Rain)
        {
            adjustedTemperature = defaultTemperature - 10.0f;
            Debug.Log("Rain temperature calculated: " + adjustedTemperature);
        }
        else if (weatherState == WeatherState.Snow)
        {
            adjustedTemperature = defaultTemperature - 20.0f;
            Debug.Log("Snow temperature calculated: " + adjustedTemperature);
        }
        ...
        fieldThermoGrid.ApplyTemperatureAdjustment(adjustedTemperature);
        // Debug.Log("Adjusted temperature applied: " + adjustedTemperature);

        // Pass the calculated adjustedTemperature to FieldThermoGrid.cs
        fieldThermoGrid.ApplyTemperatureAdjustment(adjustedTemperature);
    }
}
