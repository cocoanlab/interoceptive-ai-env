...
public class FoodCollectorAgent : Agent
{
        ...
        {
                var discreateActionOut = actionsOut.DiscreteActions[0];
                // actionsOut[0] = 0f;
                discreateActionOut = 0;
                if (Input.GetKey(KeyCode.UpArrow))
                {
                        // actionsOut[0] = 1f;
                        discreateActionOut = 1;
                        // Debug.Log("Forward!");
                }
                if (Input.GetKey(KeyCode.LeftArrow))
                {
                        // actionsOut[0] = 2f;
                        discreateActionOut = 2;
                        // Debug.Log("Left!");
                }
                if (Input.GetKey(KeyCode.RightArrow))
                {
                        // actionsOut[0] = 3f;
                        discreateActionOut = 3;
                        // Debug.Log("Right!");
                }
                if (Input.GetKey(KeyCode.Space))
                {
                        // actionsOut[0] = 4f;
                        discreateActionOut = 4;
                        // Debug.Log("Eat!");
                }

                MoveAgent(discreateActionOut);
        }
        ...
        public void MoveAgent(int action)
        {
                ...
                switch (action)
                {
                        ...
                        case 4:
                                this.isEat = true;
                                // Debug.Log("Eat!");
                                // Debug.Log("isEat: " + this.isEat);
                                // Debug.Log(this.IsEat);

                                bool onTrigger = eatRange.GetComponent<FoodEating>().IsEatTriggerOn;
                                if (onTrigger)
                                {
                                        // Debug.Log("onTrigger: " + onTrigger);
                                        EatFood(eatRange.GetComponent<FoodEating>().collide);
                                }
        ...
        {
                bool isEaten = false;
                if (this.IsAutoEat || this.IsEat)
                {
                        if (other.CompareTag("food_red"))
                        {
                                this.IncreaseLevel("food_red");
                                isEaten = true;
                        }
                        else if (other.CompareTag("food_blue"))
                        {
                                this.IncreaseLevel("food_blue");
                                isEaten = true;
                        }
                }
                ...
                if (isEaten)
                {
                        other.transform.position = new Vector3(Random.Range(-m_MyArea.range, m_MyArea.range),
                        3f, Random.Range(-m_MyArea.range, m_MyArea.range)) + m_MyArea.transform.position;
        ...
        public void SetResetParameters()
        {
                ...
                autoEat = System.Convert.ToBoolean(m_ResetParams.GetWithDefault("auto_eat", ToSingle(autoEat)));
